...
func caseC(host, path string) result {
	...
	case !accepted(valid):
		return report("C", "chunk-size accepts 'g'/'G' as 16 (F1)", "inconclusive", false,
			ev+" — valid chunked ('10') not accepted; server may not do chunked here (or not HTTP/1.1)")
	case !rejected(z):
		return report("C", "chunk-size accepts 'g'/'G' as 16 (F1)", "inconclusive", false,
			ev+" — control 'z' not rejected; server does not validate chunk sizes (can't isolate 'g')")
